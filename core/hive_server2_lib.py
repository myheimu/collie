...
from _TCLIService import TCLIService
from _TCLIService.ttypes import TOpenSessionReq, TGetTablesReq, TFetchResultsReq,\
...
'''
from _beeswax.models import Session, HiveServerQueryHandle, HiveServerQueryHistory
from _beeswax.server.dbms import Table, NoSuchObjectException, DataTable,\
...
class HiveServerClient:
  ...
  def __init__(self, query_server, user):
    ...
    self._client = thrift_util.get_client(TCLIService.Client,
                                          ...
                                          kerberos_principal="h_sns@XIAOMI.HADOOP",
                                          # use_sasl=use_sasl,
                                          use_sasl=True,
  ...
  def open_session(self, user):
    ...
    req = TOpenSessionReq(username="wenghaibin", configuration={})
    res = self._client.OpenSession(req)

    LOG.info('Opening session %s' % res.sessionHandle.sessionId)
  ...
  def get_log(self, operation_handle):
    return ''
    # TODO(yukangle)
    # The following code will crash when using hive-0.9, so currently we just return an empty string to
    # bypass it. This will be fixed in the future.
...
if __name__ == "__main__":
    #### query server ####
    query_server = dict()
    query_server['server_host'] = "lg-hadoop-hive01.bj"
    query_server['server_port'] = "9600"
    query_server['server_name'] = "collie"
    #### user ####

    #### hive server client ####
    client = HiveServerClient(query_server=query_server, user=None)
